...
#flags=("--basic-aa" "--tbaa" "--globals-aa" "--scev-aa" "--objc-arc-aa" "--cfl-steens-aa" "--external-aa" "--scoped-noalias-aa")
##flags=("--external-aa" "--scoped-noalias-aa")
...
for op in "${op_levels[@]}"
do
#  echo "" | tee  ./op-"$op"/combined_analysis.txt
  ...
  fl="$f1+$f2"
    ...
    for file in $files
    do
        opt  -enable-new-pm=0  "$f1" "$f2" -aa-eval < $file  -disable-output -stats>/dev/null 2> output1.txt
    ...
    echo "$fl no $no partial $partial may $may must $must time $SECONDS memory$peak_memory" | tee -a  ./op-"$op"/combined_analysis.txt
